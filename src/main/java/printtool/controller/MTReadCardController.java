...
public class MTReadCardController {

    Logger log = LoggerFactory.getLogger(MTReadCardController.class);

    @Autowired
    public MTReadCardController(){

        System.loadLibrary("mt/dcrf32");
        System.loadLibrary("mt/ssse32");
        System.loadLibrary("mt/fct_des");
        System.loadLibrary("mt/GetInfo");
        System.loadLibrary("mt/libghttp");
        System.loadLibrary("mt/mt_32");

        System.loadLibrary("mt/SSCardDriver");
        System.loadLibrary("mt/HttpInter_ZJZS_SB");
        System.loadLibrary("mt/ICCDevInter_ZS_SB");
    }

    @GetMapping(value = "/readCard", produces = {"application/json;charset=UTF-8"})
    public String ICReadCard(){
        Map<String, String> resMap = new LinkedHashMap<>();
        byte[] outData = new byte[1024];
        int code = MTICCInter.IC_ReadCardInfo(outData);
...
//            resMap.put("XB", sp_res[7]);
    ...
    public String ICRFReadCard(){
        Map<String, String> resMap = new LinkedHashMap<>();
        byte[] outData = new byte[1024];
        int code = MTICCInter.IC_RF_ReadCardInfo(outData);
        resMap.put("code", String.valueOf(code));
        if (code == 0){
...
//            resMap.put("YLKH", sp_res[6]);
    ...
    public String ICReadFlag(){
        Map<String, String> resMap = new LinkedHashMap<>();
        byte[] outData = new byte[1024];
        int code = MTICCInter.IC_ReadFlag(outData);
        resMap.put("code", String.valueOf(code));
        if (code == 0){
...
//            resMap.put("KTBZ", sp_res[0]);
    ...
    public String ICReadIDCard(){
        Map<String, String> resMap = new LinkedHashMap<>();
        byte[] outData = new byte[1024];
        int code = MTICCInter.IC_ReadIdCard(outData);
        resMap.put("code", String.valueOf(code));
        if (code == 0){
